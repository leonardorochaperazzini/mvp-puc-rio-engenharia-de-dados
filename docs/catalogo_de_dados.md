...
| date_key | int | FK para `dim_date` (yyyyMMdd) | 20120101–20171231 |
...
| latitude | double | Latitude (nula se inválida) | ~41.6 a 42.05 |
| longitude | double | Longitude (nula se inválida) | ~-87.95 a -87.50 |

## dim_date

| Coluna | Tipo | Descrição | Domínio |
|--------|------|-----------|---------|
| date_key | int | Chave (yyyyMMdd) | 20120101–20171231 |
| date | date | Data | 2012-01-01 a 2017-12-31 |
...
| Coluna | Tipo | Descrição | Domínio |
|--------|------|-----------|---------|
| crime_type_key | int | Chave substituta | ≥ 1 |
...
| pct_below_poverty | double | % domicílios abaixo da pobreza | ~3 a 57 |
| pct_unemployed | double | % da população 16+ desempregada | ~4 a 36 |
| pct_no_highschool | double | % 25+ sem ensino médio | ~3 a 55 |
| pct_dependent_age | double | % com menos de 18 ou mais de 64 anos | ~13 a 51 |
| per_capita_income | int | Renda per capita anual (USD) | ~8.500 a 88.700 |
